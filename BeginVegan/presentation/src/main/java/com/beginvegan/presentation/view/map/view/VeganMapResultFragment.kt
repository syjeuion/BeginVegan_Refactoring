...
import com.beginvegan.presentation.databinding.FragmentMapResultBinding
...
import com.beginvegan.presentation.view.map.viewModel.VeganMapResultViewModel
...
class VeganMapResultFragment :
    ...
    private val viewModel: VeganMapResultViewModel by viewModels()
    ...
    override fun init() {
        ...
        args.let {
            showToast("searchWord: ${it.searchWord}")
            binding.btnSearch.text = it.searchWord
        }
        // 권한 체크
...
//                binding.includedBottomSheet.tvBottomSheetTitle.text = headlineSpannable
    ...
    private fun setBottomSheet() {
        ...
        for (i in 0 until 3) {  // 예시로 3개의 탭을 추가
            val tab = binding.includedBottomSheet.tlResult.newTab()
            val customView =
                LayoutInflater.from(requireContext()).inflate(R.layout.custom_checked_tab, null)
            val checkedTextView = customView.findViewById<TextView>(R.id.tv_tab_text)
            val checkedImageView = customView.findViewById<ImageView>(R.id.iv_tab_check)
            when (i) {
                0 -> {
                    checkedTextView.text = "리뷰 많은 순"
                    checkedImageView.visibility = View.VISIBLE
                    checkedTextView.setTextColor(
                        ContextCompat.getColor(
                            requireContext(),
                            R.color.color_text_01
                        )
                    )
                    checkedTextView.setTextAppearance(R.style.body_2_regular)
                }

                1 -> {
                    checkedTextView.text = "스크랩 많은 순"
                    checkedImageView.visibility = View.GONE
                    checkedTextView.setTextColor(
                        ContextCompat.getColor(
                            requireContext(),
                            R.color.color_text_03
                        )
                    )
                    checkedTextView.setTextAppearance(R.style.body_2_regular)
                }
            ...
            binding.includedBottomSheet.tlResult.addTab(tab)
        }
        binding.includedBottomSheet.tlResult.addOnTabSelectedListener(object :
            TabLayout.OnTabSelectedListener {
            override fun onTabSelected(tab: TabLayout.Tab) {
                val checkedTextView = tab.customView?.findViewById<TextView>(R.id.tv_tab_text)
                val checkedImageView = tab.customView?.findViewById<ImageView>(R.id.iv_tab_check)
                checkedImageView?.visibility = View.VISIBLE
                ...
                viewModel.getRestaurantSearchResult(
                    0,
        ...
        binding.includedBottomSheet.tlResult.getTabAt(0)?.select()
    }
    ...
    private fun setRVAdapter() {
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            viewModel.searchResultList.collect { restaurantList ->
                if (restaurantList.isNotEmpty()) {

                    binding.includedBottomSheet.tvEmptyView.visibility = View.GONE
                    binding.includedBottomSheet.tlResult.visibility = View.VISIBLE
                    binding.includedBottomSheet.rvBottomSheetRestaurantList.visibility =
                        View.VISIBLE
                    binding.includedBottomSheet.ibFab.visibility = View.VISIBLE
                    logMessage("viewLifecycleOwner collect restaurantList $restaurantList")
                    veganMapRestaurantRVAdapter.submitList(restaurantList)
                } else {
                    binding.includedBottomSheet.tvEmptyView.visibility = View.VISIBLE
                    binding.includedBottomSheet.tlResult.visibility = View.GONE
                    binding.includedBottomSheet.rvBottomSheetRestaurantList.visibility = View.GONE
                    binding.includedBottomSheet.ibFab.visibility = View.GONE
                }
            }
        }
        ...
        veganMapRestaurantRVAdapter.setOnItemClickListener(object :
            ...
            override fun onClick(data: VeganMapRestaurant) {
                ...
                showToast("${data.name}")
                val action =
                    VeganMapResultFragmentDirections.actionVeganMapResultFragmentToRestaurantDetailFragment(
...
//            )
    ...
    companion object {
        ...
        private const val FILTER_SCRAP = "SCRAP"
        private const val FILTER_DISTANCE = "DISTANCE"
    }
